...
st.markdown(
    f"**Sector:** {sector_info} | **Sub-Sector:** {sub_sector_info} | **NSE Ticker:** {nse_ticker}"
)
...
if not df_ratios_full.empty:
    ...
    col2.metric(
        "ROCE", fmt(roce, "%"), help=f"Source year: {roce_yr}" if roce_yr else None
    )
    col3.metric(
        "Net Profit Margin",
        fmt(npm, "%"),
        help=f"Source year: {npm_yr}" if npm_yr else None,
    )
    col4.metric("D/E", fmt(de, "x"), help=f"Source year: {de_yr}" if de_yr else None)
    col5.metric(
        "Revenue CAGR 5yr",
        fmt(rev_cagr, "%"),
        help=f"Source year: {rev_yr}" if rev_yr else None,
    )
    col6.metric(
        "FCF (Latest)",
        fmt(fcf, " Cr"),
        help=f"Source year: {fcf_yr}" if fcf_yr else None,
    )
else:
...
with col_bar:
    ...
    if not df_pl.empty:
        ...
        fig_bar = go.Figure()
        fig_bar.add_trace(
            go.Bar(x=df_pl_sorted["year"], y=df_pl_sorted["sales"], name="Revenue")
        )
        fig_bar.add_trace(
            go.Bar(
                x=df_pl_sorted["year"], y=df_pl_sorted["net_profit"], name="Net Profit"
            )
        )
        fig_bar.update_layout(
            barmode="group", xaxis_title="Year", yaxis_title="Amount (Cr)"
        )
...
with col_line:
    ...
    if not df_ratios_full.empty:
        ...
        if (
            "return_on_equity_pct" in df_ratios_sorted.columns
            and "return_on_capital_employed_pct" in df_ratios_sorted.columns
        ):
            fig_line = go.Figure()
            fig_line.add_trace(
                go.Scatter(
                    x=df_ratios_sorted["year"],
                    y=df_ratios_sorted["return_on_equity_pct"],
                    mode="lines+markers",
                    name="ROE %",
                )
            )
            fig_line.add_trace(
                go.Scatter(
                    x=df_ratios_sorted["year"],
                    y=df_ratios_sorted["return_on_capital_employed_pct"],
                    mode="lines+markers",
                    name="ROCE %",
                )
            )
