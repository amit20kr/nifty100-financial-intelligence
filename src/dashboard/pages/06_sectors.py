...
if not df_companies.empty:
    df_screener = df_screener.merge(
        df_companies[["id", "company_name"]],
        left_on="company_id",
        right_on="id",
        how="left",
    )
    df_screener["company_name"] = df_screener["company_name"].fillna(
        df_screener["company_id"]
    )
else:
...
if selected_sector != "All Sectors":
    ...
    sector_medians = df_filtered.groupby("broad_sector", as_index=False).median(
        numeric_only=True
    )
    ...
    if not sector_medians.empty:
        ...
        col2.metric(
            "Median ROE",
            (
                f"{sm.get('return_on_equity_pct', pd.NA):.2f}%"
                if pd.notna(sm.get("return_on_equity_pct"))
                else "N/A"
            ),
        )
        col3.metric(
            "Median P/E",
            (
                f"{sm.get('pe_ratio', pd.NA):.2f}x"
                if pd.notna(sm.get("pe_ratio"))
                else "N/A"
            ),
        )
        col4.metric(
            "Median D/E",
            (
                f"{sm.get('debt_to_equity', pd.NA):.2f}x"
                if pd.notna(sm.get("debt_to_equity"))
                else "N/A"
            ),
        )
...
with col_tree:
    ...
    if "market_cap_crore" in df_tree.columns:
        df_tree = df_tree[df_tree["market_cap_crore"] > 0].dropna(
...
with col_bar:
    ...
    if "screener_composite_score" in df_filtered.columns:
        df_rank = df_filtered.sort_values(
            "screener_composite_score", ascending=True
        ).dropna(subset=["screener_composite_score"])
