...
if pd.isna(cf_pattern) or pd.isna(capex_intensity) or cf_pattern == '' or capex_intensity == '':
    st.info("No qualitative capital pattern available (Missing historical cashflow structure)")
else:
...
with col_cf:
    ...
    if 'cash_from_operations_cr' in df_ratios.columns:
        fig_cf.add_trace(go.Bar(
            x=df_ratios['year'], y=df_ratios['cash_from_operations_cr'],
            name='CFO (Cr)', marker_color='green'
        ))
    ...
    if 'capex_cr' in df_ratios.columns:
        ...
        fig_cf.add_trace(go.Bar(
            x=df_ratios['year'], y=df_ratios['capex_cr'],
            name='Capex (Cr)', marker_color='red'
        ))
        
    # Free Cash Flow (Source of truth from ratios)
    if 'free_cash_flow_cr' in df_ratios.columns:
        fig_cf.add_trace(go.Scatter(
            x=df_ratios['year'], y=df_ratios['free_cash_flow_cr'],
            mode='lines+markers', name='FCF (Cr)', line=dict(color='orange', width=3)
        ))
...
with col_debt:
    ...
    if 'total_debt_cr' in df_ratios.columns:
        fig_debt.add_trace(go.Bar(
            x=df_ratios['year'], y=df_ratios['total_debt_cr'],
            name='Total Debt (Cr)', marker_color='gray'
        ))
        
    if 'net_debt_cr' in df_ratios.columns:
        fig_debt.add_trace(go.Scatter(
            x=df_ratios['year'], y=df_ratios['net_debt_cr'],
            mode='lines+markers', name='Net Debt (Cr)', line=dict(color='blue', width=3)
        ))
        
    fig_debt.update_layout(barmode='overlay', xaxis_title="Year", yaxis_title="Amount (Cr)")
