...
if os.path.exists(warnings_path):
    ...
    if selected_group in warnings_df['peer_group_name'].values:
        st.warning(f"⚠️ Small Sample Warning: The '{selected_group}' peer group has fewer than 5 companies. Percentile rankings and averages may be statistically unreliable.")
...
selected_company = st.selectbox("Select Company to compare vs Group Average", options=companies_in_group)
...
st.caption("Plots the company's percentile rank against the peer group average percentile for each metric.")
...
def get_display_value(row):
    if row['metric'] == 'interest_coverage' and pd.isna(row['value']) and row['percentile_rank'] == 1.0:
        return 'Debt Free'
...
# Pivot
pivot_df = df_display.pivot(index=['company_name', 'company_id', 'is_benchmark', 'year', 'year_mismatch_flag'], columns='metric', values='display_value').reset_index()

# Sort by benchmark first, then name
pivot_df = pivot_df.sort_values(by=['is_benchmark', 'company_name'], ascending=[False, True])
...
st.caption("Note: Rows highlighted in yellow represent the benchmark company for this peer group.")
...
if not mismatches.empty:
    st.info(f"**Note:** Some companies in this group (e.g. {mismatches.iloc[0]['company_name']}) have a `year_mismatch_flag=1`, indicating their latest data year ({mismatches.iloc[0]['year']}) does not perfectly align chronologically with the rest of the peer group. Compare with caution.")
