...
if df_val.empty:
    st.warning("Valuation data not yet generated. Please run `python src/analytics/valuation.py`.")
    st.stop()
...
with col_sec:
    ...
    if pd.notna(pe_ratio) and pd.notna(pe_vs_sec):
        ...
        st.metric("Sector Median P/E", fmt(sec_pe, "x"), delta=f"{pe_vs_sec:.1f}% vs Sector", delta_color="inverse")
    else:
...
with sqlite3.connect("db/nifty100.db") as conn:
    df_mc = pd.read_sql_query("SELECT year, pe_ratio, pb_ratio FROM market_cap WHERE company_id = ? ORDER BY year ASC", conn, params=(company_id,))

if not df_mc.empty:
    fig = px.line(df_mc, x="year", y=["pe_ratio", "pb_ratio"], markers=True, title="P/E and P/B Trends")
