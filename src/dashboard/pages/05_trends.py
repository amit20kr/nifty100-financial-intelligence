...
if years_available < 5:
    ...
    st.info("CAGR metrics require 5 full years of history. This company was recently listed or data is missing.")
else:
...
with col_margins:
    ...
    if 'opm_percentage' in df_pl.columns:
        fig_margins.add_trace(go.Scatter(
            x=df_pl['year'], y=df_pl['opm_percentage'], 
            mode='lines+markers', name='Operating Profit Margin (%)'
        ))
    if 'net_profit_margin_pct' in df_ratios.columns:
        fig_margins.add_trace(go.Scatter(
            x=df_ratios['year'], y=df_ratios['net_profit_margin_pct'], 
