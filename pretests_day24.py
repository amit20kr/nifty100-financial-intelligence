...
print(
    "screener_composite_score null count:",
    engine.df["screener_composite_score"].isna().sum(),
)
...
metrics_df = pd.read_sql_query(
    "SELECT DISTINCT metric FROM peer_percentiles ORDER BY metric", conn
)
print(metrics_df["metric"].tolist())

print("\n=== PRETEST 4: Peer Benchmark Counts ===")
benchmarks_df = pd.read_sql_query(
    "SELECT peer_group_name, COUNT(*) as c FROM peer_groups WHERE is_benchmark=1 GROUP BY peer_group_name",
    conn,
)
print(benchmarks_df)
