...
def run_qa():
    ...
    for ticker in test_tickers:
        ...
        assert duration < 3.0, f"Profile for {ticker} took {duration:.2f}s (must be < 3s)"
        print(f"[{ticker}] Profile calls took: {duration:.3f}s - PASS")
        
    print("\n--- 2. Screener Extreme Stress Test ---")
    # Engine initialization benchmark
    eng_start = time.time()
    engine = FilterEngine(project_root / "db/nifty100.db", project_root / "config/screener_config.yaml")
    eng_end = time.time()
    ...
    try:
        ...
        print(f"Extreme payload resulted in {len(res)} matches without crashing. - PASS")
    ...
    for ticker in sparse_tickers:
        try:
            ...
            print(f"[{ticker}] db.py getters handled missing/sparse data gracefully. - PASS")
        except Exception as e:
