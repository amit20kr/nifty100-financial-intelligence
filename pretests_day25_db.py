...
def run_pretests():
    ...
    df_ratios = pd.read_sql(
        "SELECT company_id, COUNT(*) as years FROM financial_ratios GROUP BY company_id",
        conn,
    )
    ...
    df_labels = pd.read_sql(
        "SELECT company_id, cashflow_pattern_label, capex_intensity_label FROM financial_ratios WHERE year = (SELECT MAX(year) FROM financial_ratios)",
        conn,
    )
    missing_cashflow = df_labels[
        df_labels["cashflow_pattern_label"].isna()
        | (df_labels["cashflow_pattern_label"] == "")
    ]
    missing_capex = df_labels[
        df_labels["capex_intensity_label"].isna()
        | (df_labels["capex_intensity_label"] == "")
    ]
    ...
    df_mc = pd.read_sql(
        "SELECT * FROM market_cap WHERE year = (SELECT MAX(year) FROM market_cap)", conn
    )
    ...
    df_sectors = pd.read_sql(
        "SELECT broad_sector, COUNT(*) as count FROM sectors GROUP BY broad_sector",
        conn,
    )
    ...
    df_cf = pd.read_sql(
        "SELECT company_id, free_cash_flow_cr FROM financial_ratios", conn
    )
