...
def run_pretests():
    ...
    df_counts = pd.read_sql(
        "SELECT company_id, COUNT(*) as years_count FROM market_cap GROUP BY company_id",
        conn,
    )
    ...
    df_spot = pd.read_sql(
        "SELECT company_id, year, pe_ratio, market_cap_crore FROM market_cap WHERE company_id IN ('RELIANCE', 'JIOFIN') ORDER BY company_id, year DESC",
        conn,
    )
    print(df_spot)
