...
years = pd.read_sql_query('SELECT DISTINCT year FROM financial_ratios ORDER BY year', conn)
print(years['year'].tolist())

print('\n--- NSE PROFILES (LIMIT 5) ---')
nse_profiles = pd.read_sql_query('SELECT id, company_name, nse_profile FROM companies LIMIT 5', conn)
print(nse_profiles)
null_nse = pd.read_sql_query('SELECT COUNT(*) as c FROM companies WHERE nse_profile IS NULL OR nse_profile = ""', conn)
...
print('composite_quality_score in columns:', 'composite_quality_score' in engine.df.columns)
print('screener_composite_score in columns:', 'screener_composite_score' in engine.df.columns)
